...
def DNN(X):
    model = Sequential()
    # The Input Layer :
    model.add(Dense(50, kernel_initializer='normal',input_dim = X.shape[1], activation='relu'))
    # The Hidden Layers :
...
for train_index,test_index in skf.split(X, y):
    ...
    y_test = (y_test * Xstds[48]) + Xmeans[48]
    predicted_y = (predicted_y * Xstds[48]) + Xmeans[48]
...
yy = (y * Xstds[48]) + Xmeans[48]
cv_sety = cv_set 


print("Overall R: " + str(pearsonr(yy, cv_sety)))
...
print("Overall MAPE: " + str(mean_absolute_percentage_error(yy, cv_sety)))
...
# plt.hist(x=me,  bins=15, color='#0504aa', alpha=0.7, rwidth=0.85)
# plt.grid(axis='y', alpha=0.6)
# plt.grid(axis='x', alpha=0.6)
# plt.xlabel("Value (mmHg)",fontsize=10)
# plt.ylabel("Frequency (%)",fontsize=10)
# plt.savefig("imgs/"+ "DP_Hist"+".pdf", bbox_inches='tight', dpi=320)  
# plt.show()
...
# from graph import*
# bland_altman_plot(yy, cv_sety, "RFE_B_DP")
# act_pred_plot(yy, cv_sety,R_2,mae,"RFE_R2_DP")